...
class OhemCELoss(nn.Module):
    ...
    """
    # 200000, 10000
    def __init__(self, thresh=1, n_min=200000, ignore_lb=255, *args, **kwargs):
        super(OhemCELoss, self).__init__()
        # self.thresh = -torch.log(torch.tensor(thresh, dtype=torch.float)).cuda()  # 将输入的概率 转换为loss值
        self.thresh =torch.tensor(thresh, dtype=torch.float).cuda()  # 将输入的概率 转换为loss值
        # print(self.thresh)
        self.n_min = n_min
    ...
    def forward(self, logits, labels):
        ...
        self.thresh =torch.sqrt(torch.mean(loss))
        # self.thresh =torch.mean(loss)
        if loss[self.n_min] > self.thresh:  # 当loss大于阈值(由输入概率转换成loss阈值)的像素数量比n_min多时，取所有大于阈值的loss值
